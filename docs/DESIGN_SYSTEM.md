...
- 진행률: 현재 범위가 승인된 프로젝트만 숫자로 표시한다. 미승인은 `계획 미승인`, 범위 변경은 `계획 재승인 필요`, 승인 필드 없는 구버전은 `기준 미확인`이다. 기본 WBS 비율과 선택 마일스톤 가중 정책을 자동 전환하지 않는다. 합성 검증 업무는 제외하고 다른 자동 구성 업무는 명시 확인 후 승인한다.
- 계획 승인: 프로젝트 개요의 한 disclosure에서 범위·계산 방식·가중치·제외 사유·자동 구성 확인·최근 승인 이력을 관리한다. 실패 초안을 유지하고 응답 유실은 같은 요청 재시도 또는 명시적 재조회로 해결한다. 업무 상태 변경은 진척만 갱신하고 범위 변경은 재승인을 요구한다.
- 자동 리포트는 `현재 WBS 진척`과 조회 기준 시각을 표시한다. 프로젝트 요약의 비율·출처·승인 버전을 그대로 사용하고 미승인·재승인 필요·실제 0%·미확인을 구분한다. 기간 내 문서 추출 잔여/지연 항목은 전체 WBS 잔여량과 별도로 이름 붙인다.
- 기간 리포트는 실제 상태 전이·로그의 막힘·기간 내 수정된 확인 성과·현재 잔여 WBS를 구분한다. 현재 상태와 과거 전이를 동시에 표시하고 해결 여부·성과 달성일을 추정하지 않는다. 조회 시각·시간대·종료 미포함 경계·내용 해시를 표시하며, 필터 변경 후 이전 응답이나 실패한 재생성이 과거 결과를 현재 결과로 보이게 해서는 안 된다. 목록은 고정된 최대 높이 안에서 모든 행에 접근 가능하게 한다.
- 리포트의 `생성·보관`은 서버에서 만든 결과를 저장한다. 같은 화면의 보관함에서 페이지 단위로 조회하며 저장 시각과 조회 기준 시각을 구분한다. 보관본의 WBS·상태는 `보관 당시`로 표시하고 현재 DB 값을 섞지 않는다. 저장 실패 재시도는 동일 요청 ID를 유지하며, 다른 보관본을 여는 중 도착한 이전 응답은 무시한다. 원본 삭제와 보관본 삭제는 별개이고 보관본 영구 삭제는 명시 확인을 받는다. 다운로드·복사는 저장된 Markdown 그대로다.
...
- 데이터 출처: 공통 progress formatter는 현재 승인된 프로젝트만 평균에 포함하고 출처 필드 누락은 `기준 미확인`으로 표시한다. 사용자가 명시 확인한 자동 구성 WBS도 승인 후에는 산정에 포함하며 출처를 숨기지 않는다. 개별 마일스톤은 등록 WBS 기준의 참고 비율로 프로젝트 승인 진척과 구분한다. 프로젝트 상태와 계산된 WBS 비율을 동일시하지 않는다. 상세에서는 `수치 산정 근거`를 펼쳐 분모·분자·출처·계산식을 확인한다.
...
- 개발·운영 상태: 프로젝트 목록·대시보드·상세에서 두 축을 구분한다. 개요의 상태 확인은 별도 카드 없이 구분선으로 나누며, 사유·미완료 업무 처리 사유·선택 종료일·최근 확인 이력을 제공한다. 일반 정보 편집에 상태 입력을 중복 배치하지 않는다. 충돌 재조회는 최신 상태 선택으로 초기화하되 작성 사유는 보존하고, 저장 결과 불명확 시 같은 요청만 재시도한다.

## 2. UI 원칙

업무 완료 이력은 기존 WBS 수정 양식의 disclosure 한 곳에서 조회한다. 완료 처리일 미확인은 날짜를 생성하지 않고 그대로 표시하며, 이력 조회 실패는 빈 이력과 구분한다. 상태 저장 중 WBS 조작은 잠그고, 충돌·응답 유실 후 재조회는 상태/버전만 갱신하여 작성 중인 제목·설명·사유를 보존한다. 대시보드 주간 완료는 수정일이 아닌 기록된 완료 처리 시각(한국 시간) 기준이다.
