...
test("production compose runs release commands without source mounts or public internal ports", () => {
  ...
  assert.match(deployScript, /up -d --wait db/);
  assert.match(deployScript, /Usage: \$0 <verified-commit-sha>/);
  assert.match(deployScript, /\^\[0-9a-fA-F\]\{40\}\$/);
  assert.match(deployScript, /git checkout --detach "\$DEPLOY_SHA"/);
  assert.match(deployScript, /git rev-parse HEAD/);
  assert.match(deployScript, /python scripts\/migrate_db\.py --preflight/);
  assert.match(deployScript, /exec -T backend python scripts\/migrate_db\.py/);
  assert.doesNotMatch(deployScript, /for migration in infrastructure\/postgres\/init\/\*\.sql/);
  assert.doesNotMatch(deployScript, /psql/);
  assert.doesNotMatch(deployScript, /sync_github_data\.py/);
  assert.doesNotMatch(deployScript, /--cleanup-samples/);
  assert.match(workflow, /DEPLOY_SHA:\s*\$\{\{ github\.sha \}\}/);
  assert.match(workflow, /"bash -s -- \$DEPLOY_SHA" < scripts\/deploy_oracle\.sh/);
  assert.doesNotMatch(workflow, /secrets\.WORKTRACE_GH_TOKEN/);
  assert.doesNotMatch(workflow, /secrets\.WORKTRACE_WEBHOOK_SECRET/);
  assert.doesNotMatch(workflow, /bash scripts\/deploy_oracle\.sh/);
  ...
  assert.match(compose, /python scripts\/migrate_db\.py && exec uvicorn/);
