...
자동 리포트의 `현재 WBS 진척`도 동일한 집계를 사용합니다. 프로젝트 상태·문서 수·업무 로그 수에 임의 점수를 더하지 않습니다. 선택 기간의 자료에 연결된 프로젝트를 대상으로 **리포트 생성 시점의 현재 WBS**를 조회하며 과거 종료일의 진척 스냅샷이 아닙니다. API와 Markdown에 기준 시각·완료/전체 수·산정 방식·자동 구성 여부·승인 상태와 버전을 포함합니다. `계획 미승인`, `계획 재승인 필요`, `기준 미확인`과 승인한 업무의 실제 `0%`를 구분합니다. `suggested_progress_percent`는 호환용 nullable 별칭이며 추정값이 아닙니다. 잔여/지연 목록은 여전히 **기간 내 문서 추출 항목**이므로 전체 WBS 잔여량과 구분합니다.
