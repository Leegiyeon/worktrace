...
- `/reports/automatic` 자체는 저장하지 않는 조회 API로 유지합니다. 화면의 `생성·보관`은 `/reports/snapshots`에서 같은 생성기를 사용하고 결과 전체와 Markdown을 DB에 고정합니다. 보관함 재조회는 원본을 다시 집계하지 않으며, 보관 당시 값이지 현재 상태나 선택 기간 종료일의 상태 복원이 아닙니다.
- `020_report_snapshots.sql` 적용이 필요합니다. 같은 소유자·요청 UUID의 재시도는 최초 저장 결과를 반환하고, 다른 기간/유형으로 재사용하면 409입니다. 새 UUID는 내용이 같아도 새 보관본을 만듭니다. 목록은 페이지 조회, 상세·삭제는 소유자 범위로 제한합니다. 내용 해시와 별도로 전체 저장 payload의 SHA-256을 검사하지만 외부 서명이나 변조 불가능한 감사 저장소는 아닙니다.
- **원본 프로젝트·업무·문서를 삭제해도 이미 보관한 리포트는 남습니다.** 기밀 자료를 완전히 제거할 때는 관련 보관본도 별도로 삭제해야 합니다. 저장본 수정은 지원하지 않고 명시 확인 후 삭제할 수 있습니다. 보고서에 포함된 로그/성과/문서 요약은 보존하지만 원본 파일·전체 문서 텍스트·기간 밖 연결 근거 전문까지 복제하는 기능은 아닙니다. AI 호출 없이 생성하며 경력 생성과 자동 연결하지 않습니다.
...
현재 리포트 화면은 `POST /reports/snapshots`로 생성·보관합니다. `report_type`은 `daily`,
...
보관 API는 동일한 인증 헤더를 사용합니다.
- `POST /reports/snapshots`: `report_type`, `start_date`, `end_date`, UUID `request_id`를 전달합니다. 서버가 생성한 리포트만 저장하며 클라이언트의 리포트 본문은 받지 않습니다.
- `GET /reports/snapshots?limit=20&offset=0`: 저장 시각 내림차순 목록과 전체 수를 반환합니다. `limit`은 1~100입니다.
- `GET /reports/snapshots/{snapshot_id}`: 저장 당시 결과와 Markdown을 그대로 반환합니다.
- `DELETE /reports/snapshots/{snapshot_id}`: 해당 보관본만 영구 삭제합니다. 원본 업무·성과에는 영향을 주지 않습니다.
- 기존 `/reports/automatic`과 `/reports/weekly`는 저장 없이 생성하는 호환 API로 남습니다.
