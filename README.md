...
자동 리포트의 `현재 WBS 진척`도 동일한 집계를 사용합니다. 프로젝트 상태·문서 수·업무 로그 수에 임의 점수를 더하지 않습니다. 선택 기간의 자료에 연결된 프로젝트를 대상으로 **리포트 생성 시점의 현재 WBS**를 조회하며 과거 종료일의 진척 스냅샷이 아닙니다. API와 Markdown에 기준 시각·완료/전체 수·산정 방식·자동 구성 여부를 포함합니다. 미산정은 `산정 전`, 요약 미확인은 `기준 미확인`, 실제 0%는 `0%`로 구분합니다. `suggested_progress_percent`는 호환용 nullable 별칭이며 추정값이 아닙니다. 잔여/지연 목록은 여전히 **기간 내 문서 추출 항목**이므로 전체 WBS 잔여량과 구분합니다.
...
- 완료 확인은 업무 상태·프로젝트 상태·기존 진척률을 변경하지 않습니다. 과거 `milestone-validation:*` 자동 검증 WBS는 수정/삭제를 차단하고 확인 이력으로 자동 승격하지 않습니다. 완료 검토의 실제 WBS 수/차단 조건에서는 이를 제외하지만 기존 프로젝트 진척 산식은 보존하므로 두 수치는 다를 수 있습니다. 승인 계획 기반 산식과 범위 제외 사유는 후속 작업입니다.
