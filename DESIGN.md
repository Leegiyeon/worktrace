...
- Honest scope: stale AI reviews remain visible as advisory history; neither current nor stale AI reviews gate manual confirmation. Current user-confirmed milestones are not overwritten by batch AI calls; unavailable data is shown as loading, empty, error, or disconnected rather than implied success.
...
- Numeric provenance: only currently approved project plans produce percentages and enter cross-project averages. Derived WBS requires explicit acknowledgement at approval, and its provenance remains visible. Missing/stale approval is not zero. Project detail exposes counted totals, derived totals, selected policy and approval version. Milestone ratios remain registered-work indicators rather than independent plan approvals. Model confidence is not presented as measured accuracy; task modification time is not labeled completion time.
...
## Interaction states
- Manual milestone confirmation is separate from synthetic legacy WBS and advisory AI. The existing completion-review row owns reason/evidence inputs, current-vs-stale confirmation and recent history. Failed requests retain drafts and request identity; conflicts require an explicit evidence reload. Confirmation never changes WBS status or progress. Criteria and remaining substantive counted tasks are server-side gates; excluded tasks additionally require a current project-plan approval with exclusion rationale.
- Automatic reports use current project-summary WBS progress with an as-of timestamp, not guessed percentages or historical-period snapshots. Preserve unknown, unscoped, real-zero and derived-reference states; label period-selected document tasks separately from whole-project WBS.
- Period report activity separates recorded task transitions, log blockers and modified confirmed outcomes from current remaining WBS. All sections share one read snapshot with timezone, half-open timestamp bounds and a content fingerprint. A fingerprint is not persisted report history. Changing filters invalidates the prior result and in-flight responses; retry failures never relabel old output as current. Unknown issue resolution and outcome achievement dates stay unknown.
- Report generation in the UI creates a saved version. Keep generation and the paginated archive in the existing reports route; never regenerate a saved version when opening it. Show the saved as-of time and label remaining WBS/status as historical snapshot values, not live state. Preserve a failed request's idempotency key for retry. Copy/download the exact stored Markdown, and require explicit confirmation for permanent archive deletion; deleting source records does not erase saved versions.
- WBS transition history: completed_at is the latest recorded transition into done, never a copy of updated_at or an inferred historical date. Reopening clears the current completion timestamp while retaining history. One disclosure in the existing task editor shows recent status transitions and optional reasons; do not add a parallel task management screen. Status writes carry the fetched version; conflicts or uncertain edit responses require reloading current status while retaining the user's text. Dashboard weekly completions use Monday-to-now in Asia/Seoul, exclude unknown timestamps and commit-derived activities, and are distinct from approved WBS progress.
...
- Progress constraints: a project needs explicit approval of its current WBS scope and policy. Default policy is WBS count ratio; milestone weighting is an explicit alternative requiring complete assignment, nonempty weighted milestones and a 100% weight sum. Never switch formulas implicitly. Missing/stale approval yields a null percentage, not zero. Legacy synthetic validation tasks are excluded; other derived tasks require explicit review acknowledgement. Scope exclusions require a reason. Status transitions update progress without invalidating approval; scope/text/weight edits require reapproval.
- Plan review: one disclosure in the existing project overview owns scope preview, policy selection, reason, excluded-work rationale, derived-work acknowledgement and recent approval history. Keep drafts on failure, require explicit reload after conflict, and preserve exact pending payload for uncertain-save retries. Existing projects start unapproved without changing their tasks or project lifecycle.
